import { Central, ControllerMixinDatabase, ControllerMixinMime, ControllerMixinView, ControllerMixinViewState, ORM } from '@lionrockjs/central';
...
export default class ControllerAccount extends Controller {
    ...
    constructor(request, opts = {}) {
        super(request);
        const { databaseMap = new Map(), allowRoles = ['*'], rejectLanding = '/login', layout = 'layout/account', } = opts;
        const dbMap = this.state.get(ControllerMixinDatabase.DATABASE_MAP);
        dbMap.set(Central.config.auth.databaseMapName, Central.config.auth.databaseMap.get(Central.config.auth.databaseMapName));
        databaseMap.forEach((value, key) => {
            dbMap.set(key, value);
        });
        this.state.set(ControllerMixinViewState.LAYOUT_FILE, layout);
    ...
    async before() {
        this.state.get(ControllerMixinViewState.LAYOUT).data.user_role = this.state.get(ControllerState.REQUEST).session.user_role;
    }
    async action_index() {
        const userId = this.state.get(ControllerState.REQUEST).session.user_id;
